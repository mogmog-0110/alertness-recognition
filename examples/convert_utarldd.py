...
配布フォルダの構造とファイル名は手元で未確認。下の LABEL_BY_STEM と walk() が想定を
一箇所に集めているので、実物を見てここだけ書き換えれば済むようにしてある。想定は
「被験者フォルダの中に 0.* / 5.* / 10.* という動画が1本ずつ」。ファイル名からラベルを
取れない構造だった場合は walk() を書き換える。

## アノテ規約: 3クラス → 4段階

正準ラベルは4段階なので、3クラスのどれかは必ず余る。ここでは low vigilant を medium に
写し、low を空けている。「low vigilant」は本人が運転を続けられる程度の低覚醒であり、
警告の第一段階(low)より一段重い状態と読むのが妥当と判断した。逆に low へ寄せる規約も
あり得るので、決めたら docs/annotation-guide.md の眠気の表に書くこと。ここを変えるなら
LABEL_BY_STEM の一箇所だけでよい。

## ⚠ このデータセットの弱点（学習時に効いてくる）
