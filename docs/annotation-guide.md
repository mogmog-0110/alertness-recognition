...
`examples/convert_drozy.py` は、10秒のPSG窓を1秒ずつ進めて連続眠気スコア(CDS)を作る。
EEGのθ・α・DI、EOGのSEM・瞬目時間・500ms以上の長時間イベントは眠気増加方向、βは
覚醒方向として統合する。被験者ごとにPVT1冒頭を基準化し、PVT反応時間とlapse率はLoD境界の
補正だけに使う。KSSは生成結果の妥当性確認に使い、CDSやラベルを直接変更しない。

| 段階 | PSG上の操作的な意味 |
|---|---|
| none | PVT1覚醒基準に近く、β低下や眠気方向の複合変化がない |
| low | α・DIまたは瞬目時間に軽い眠気方向の変化がある |
| medium | θ・SEMを含む複数特徴が眠気方向へ継続して変化する |
| high | 強いθ優勢または500ms以上の長時間閉眼相当イベントが継続する |

CDS境界、特徴重み、時間平滑化は `config/default.yaml` の `drozy` が正準である。5秒未満の
遷移は平滑化で前後へ統合する。PVT1基準、必須PSGチャンネル、同期timestampsのいずれかが
欠けるセッションにはラベルを付けず、他セッションや `none` で補完しない。未校正または
変換時にスキップされた区間を学習データへ混入させない。
