...
評価軸ごとに音を変えるため、kind を指定して別パターンの WAV を作る。画面を見なくても
聞き分けられるよう、高さの向きだけでなくリズムと長さを変える（端末の web/sound.js と同じ形）。
- drowsy（眠気）: 長めのピーを 2 回（ピーピー）。
- distracted（脇見）: 短いピを 3 回（ピピピ）。
- inattentive（注意散漫）: さらに短いピを 5 回、速く（ピピピピピ）。
...
# kind ごとの (周波数Hz, 長さ秒) の並び。周波数 0 は無音の間。
_PATTERNS = {
    "drowsy": ((784.0, 0.38), (0.0, 0.16), (784.0, 0.38)),
    "distracted": ((988.0, 0.1), (0.0, 0.09)) * 2 + ((988.0, 0.1),),
    "inattentive": ((880.0, 0.06), (0.0, 0.05)) * 4 + ((880.0, 0.06),),
