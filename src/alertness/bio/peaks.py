...
体動の乗った実データでは、呼吸や動きによるベースライン変動が拍より振幅が大きく、
区間ごとの振幅も数倍に振れる。全体を std で正規化して固定しきい値で山を選ぶと、
(1) 拍より振幅の大きい変動の峰が拍として採られ、(2) 振幅の大きい区間にスケールを
引かれて静かな区間の拍がしきい値を下回る。拍を取りこぼすと拍間隔が倍に伸びるので、
RMSSD は拍間隔と同じ桁まで跳ね、推定心拍は実際より低く出る。

そこで3段構えにする。numpy のみで完結させる。

  1. 帯域通過   … 心拍帯の外（ベースライン変動と高周波雑音）を落とす
  2. 局所正規化 … 数拍ぶんの移動RMSで割り、区間ごとの振幅差を吸収する
  3. 不応期     … 支配周波数から推定した拍間隔を基準にし、近すぎる山を捨てる

返り値の契約は「拍のサンプル位置」なので、さらに頑健にしたくなったらこの関数だけを
差し替えればよい。
"""
...
# 不応期の上限を決める心拍[bpm]。「これより速い拍は潰さない」という下限保証。
# 支配周波数は体動が乗ると心拍より遅い側へ引かれることがあり、その間隔から不応期を
# 作ると本物の拍まで抑え込んでしまう。
_REFRACTORY_MAX_BPM = 120.0
# 局所振幅の下限。無音区間で 0 に近いスケールで割ると雑音が増幅されるのを防ぐ。
_SCALE_FLOOR_RATIO = 0.3
# 帯域通過の前に反射パディングする長さ（信号長に対する割合）。
# FFT は信号を周期的とみなすので、窓の端と端が繋がらないと不連続になり、そのリンギングが
# 端の拍位置をずらす。
_PAD_RATIO = 0.25
