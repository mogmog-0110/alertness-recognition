...
前提: 交感神経が高ぶると発汗が増え、皮膚コンダクタンス(tonic SCL)が上がる。発汗は
ゆっくり変わるので、心拍変動より体動に強い。

SCL の絶対値は個人差が桁で違うので、被験者ごとに安静を基準にし、その人の EDA の振れ幅で
正規化した「安静からどれだけ上がったか(0..1)」にそろえる。段階のしきい値はこの相対値に
対して置く。窓ごとに写すので時間変化を持つ。

しきい値は呼び出し側で決める（ここは写像だけ）。段階を細かく分けず、落ち着き/上昇の
2値に束ねて使う前提（細かい段階は境界の置き方が恣意的になる）。
"""
