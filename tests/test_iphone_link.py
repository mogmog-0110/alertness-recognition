...
from alertness.sources.iphone_ws import IPhoneLink, IPhoneSource
...
@pytest.fixture
def link():
    made = IPhoneLink("127.0.0.1", 0)  # 0 で空いている番号を借りる
    made.wait_ready()
    yield made
    made.close()
...
def test_the_capture_time_comes_from_the_device(link):
    ...
    frame = next(islice(IPhoneSource(link).frames(), 1))
    device.close()
...
def test_only_the_newest_frame_survives(link):
    ...
    frames = list(islice(IPhoneSource(link).frames(), 1))
    device.close()
...
def _stamped(link: IPhoneLink, served: int) -> tuple[int, float]:
...
def test_a_busy_port_is_reported(link):
    ...
    second = IPhoneLink("127.0.0.1", link.port)
...
def _iphone_config(port: int) -> dict:
    return {
        ...
        "feedback": {"window": False, "iphone_features": ["ear"]},
...
def test_the_source_and_the_sink_share_one_connection():
    ...
    try:
        sent = [s for s in sinks._sinks if isinstance(s, IPhoneSink)]
        assert len(sent) == 1
