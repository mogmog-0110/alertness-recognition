"""DROZYの正式な生体信号・manifest変換経路を検証する統合寄りの回帰テスト。

EDFチャンネルの別名解決と実コンテナ読込、EEG/EOG窓特徴、PVT公式時刻形式と異常入力、PVT1
基準化、LoD境界補正、時間平滑化、drowsinessだけを持つmanifestの往復を検証する。合成信号と
一時ファイルを基本とし、pyedflibやローカルの実DROZYデータが利用できる場合だけ該当テストを
実行するため、通常のCIでも追加データなしで契約を確認できる。

軽量な公開APIとパイプライン各段の基本動作は ``test_drozy.py``、ここでは公式フォーマットに
近い入出力と、欠損を推測で補わず拒否する安全側の振る舞いを主に担当する。
"""
